...
import com.chunkworks.rangedweaponsmod.Handling;
import com.chunkworks.rangedweaponsmod.ModData;
import com.chunkworks.rangedweaponsmod.ModItems;
import net.minecraft.resources.ResourceLocation;
import com.chunkworks.rangedweaponsmod.Gunnery;
import com.chunkworks.rangedweaponsmod.GunItem;
import com.chunkworks.rangedweaponsmod.MagazineFedWeapon;
...
import com.chunkworks.rangedweaponsmod.MagazineItem;
 ...
 * aside, or stays shut with no room. Loose mode (the server's feed): a
 * magazine-fed gun loads loose rounds in its change time and conjures no
...
public final class GunneryGameTests {
    ...
    // --- loose mode: every gun loads loose rounds, from the pockets then the bags
    // These run in the "loose" batch, whose before-batch hook sets the mode.

    /** A bag of {@code tier} with {@code rounds} in storage cell {@code cell}, identified and revisioned as Backpacks+ does it. */
    private static ItemStack bag(net.minecraft.world.item.Item tier, int cell, ItemStack rounds) {
        ItemStack bag = new ItemStack(tier);
        BagContents.identify(bag);
        NonNullList<ItemStack> cells = BagContents.copy(bag);
        cells.set(cell, rounds);
        BagContents.store(bag, cells);
        return bag;
    }

    @GameTest(template = "arena")
    public void roundsStackToTheGamesCeiling(GameTestHelper helper) {
        for (Item round : List.of(ModItems.SMALL_ROUND.get(), ModItems.ROUND.get(), ModItems.SHELL.get(), ModItems.SLUG.get())) {
            helper.assertValueEqual(new ItemStack(round).getMaxStackSize(), Item.ABSOLUTE_MAX_STACK_SIZE, round + " stacks to the game's ceiling");
        }
        helper.assertValueEqual(Item.ABSOLUTE_MAX_STACK_SIZE, 99, "which is ninety-nine");
        helper.succeed();
    }
    ...
    public void looseModeLoadsLooseRoundsIntoAMagazineFedGunInItsChangeTime(GameTestHelper helper) {
        ...
        helper.runAtTickTime(2, () -> {
            Reload reload = g.gun().get(ModData.RELOAD.get());
            helper.assertTrue(reload != null && !reload.swap(), "a reload started on the empty trigger");
            helper.assertValueEqual(reload.durationTicks(), RELOAD_TICKS, "the gun's change time, not a tick a round");
        });
        helper.runAtTickTime(RELOAD_TICKS + 2, () -> {
            helper.assertValueEqual(g.weapon().rounds(g.gun()), 75, "a full store, no magazine needed");
            helper.assertValueEqual(g.weapon().capacity(g.gun()), 75, "the profile's capacity");
            helper.assertTrue(Magazines.inserted(g.gun()).isEmpty(), "and none was conjured");
            helper.assertValueEqual(g.player().getInventory().countItem(ModItems.ROUND.get()), 25, "seventy-five taken");
            helper.succeed();
        });
    }
    ...
    public void looseModeHandsAMagazineBackWithItsLoad(GameTestHelper helper) {
        ...
        driveTicks(helper, g, 1, 1);
        helper.runAtTickTime(2, () -> {
            helper.assertTrue(Magazines.inserted(g.gun()).isEmpty(), "the box came out on first sight");
            helper.assertValueEqual(g.weapon().rounds(g.gun()), 0, "leaving the store empty");
            ItemStack kept = g.player().getInventory().getItem(1);
            helper.assertTrue(kept.is(ModItems.MACHINE_GUN_BOX.get()), "into the first free slot");
            helper.assertValueEqual(Magazines.contents(kept).rounds(), 20, "load and all");
            helper.succeed();
        });
    }
    ...
    public void looseModeShiftRChangesKindAndAMagazineIsNotAmmunition(GameTestHelper helper) {
        ...
        driveTicks(helper, g, 1, 1);
        helper.runAtTickTime(2, () -> {
            helper.assertValueEqual(g.weapon().rounds(g.gun()), 10, "the loose rounds stay loose: nothing adopted");
            helper.assertTrue(Magazines.inserted(g.gun()).isEmpty(), "no magazine conjured");
            helper.assertValueEqual(PlayerGunnery.countAmmo(g.player(), g.weapon(), g.gun()), 0, "the loaded kind is not carried, and the box does not count");
            PlayerGunnery.onReloadKey(g.player(), true);
        });
        driveTicks(helper, g, 3, RELOAD_TICKS + 3);
        helper.runAtTickTime(RELOAD_TICKS + 4, () -> {
            helper.assertValueEqual(g.weapon().loadedAmmo(g.gun()).orElseThrow(), Items.IRON_NUGGET, "the swap loaded the other kind");
            helper.assertValueEqual(g.weapon().rounds(g.gun()), 30, "all thirty");
            helper.assertValueEqual(g.player().getInventory().countItem(ModItems.ROUND.get()), 10, "the ten came back to the inventory");
            helper.assertValueEqual(Magazines.contents(g.player().getInventory().getItem(4)).rounds(), 75, "the box untouched");
            helper.succeed();
        });
    }
    ...
    public void looseModeDrawsFromTheWornBagAfterThePocketsAndNeverFromAMount(GameTestHelper helper) {
        ...
        NonNullList<ItemStack> cells = BagContents.copy(bag);
        cells.set(mount, new ItemStack(ModItems.ROUND.get(), 9));
        helper.assertTrue(BagContents.store(bag, cells), "nine rounds sit on a small mount: gear, not ammunition");
        long revision = BagContents.revision(bag);
        g.player().getInventory().setItem(BagLocations.CHEST, bag);
        helper.assertValueEqual(PlayerGunnery.countAmmo(g.player(), g.weapon(), g.gun()), 65, "loose mode reaches the bag's storage too, not its mount");
        PlayerGunnery.onTrigger(g.player(), true);
        driveTicks(helper, g, 1, RELOAD_TICKS + 1);
        helper.runAtTickTime(RELOAD_TICKS + 2, () -> {
            helper.assertValueEqual(g.weapon().rounds(g.gun()), 65, "five from the pockets and sixty from the bag");
            helper.assertValueEqual(g.player().getInventory().countItem(ModItems.ROUND.get()), 0, "the pockets went first and are empty");
            ItemStack worn = g.player().getInventory().getItem(BagLocations.CHEST);
            NonNullList<ItemStack> after = BagContents.copy(worn);
            helper.assertTrue(after.get(0).isEmpty(), "the storage cell is empty");
            helper.assertValueEqual(after.get(mount).getCount(), 9, "the mounted rounds are untouched");
            helper.assertValueEqual(BagContents.revision(worn), revision + 1, "one withdrawal, one revision");
            helper.succeed();
        });
    }
